//! create_launch: mint the fixed supply into the reserve, revoke the mint authority,
//! create the launch's token and quote vaults, write Launch state.
//!
...
//!  3 reserve_vault       writable, ATA(launch, token_mint)
...
//!  8 token_2022_program  Token-2022 program (used only when the quote is Token-2022)
//!  9 ata_program
//! 10 system_program
//! 11 rent sysvar
...
pub fn process(accounts: &[AccountInfo], args: &[u8]) -> ProgramResult {
    ...
    // Mint: launch PDA is the temporary authority, revoked below.
    cpi::create_mint(creator, token_mint, decimals, &launch_key, token_program)?;
    cpi::create_ata_idempotent(creator, reserve_vault, launch, token_mint, system_program, token_program, ata_program)?;
    ...
    cpi::mint_to(token_mint, reserve_vault, launch, supply, &[ls.signer()])?;
    cpi::revoke_mint_authority(token_mint, launch, &[ls.signer()])?;
